...
SHOW_DATETIME=$(tmux show-option -gv @tmux-status-line_show_datetime 2>/dev/null)
...
date_format=$(tmux show-option -gv @tmux-status-line_date_format 2>/dev/null)
time_format=$(tmux show-option -gv @tmux-status-line_time_format 2>/dev/null)
...
echo "${RESET}#[fg=${THEME[foreground]},bg=${THEME[brightblack]}]${date_string} ${separator}${time_string}"
