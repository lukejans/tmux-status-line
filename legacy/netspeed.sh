...
# Check if enabled
ENABLED=$(tmux show-option -gv @tmux-status-line_show_netspeed 2>/dev/null)
[[ ${ENABLED} -ne 1 ]] && exit 0
...
INTERFACE=$(tmux show-option -gv @tmux-status-line_netspeed_iface 2>/dev/null)
# Show IP address
SHOW_IP=$(tmux show-option -gv @tmux-status-line_netspeed_showip 2>/dev/null)
# Time between refresh
TIME_DIFF=$(tmux show-option -gv @tmux-status-line_netspeed_refresh 2>/dev/null)
...
if [[ -z $INTERFACE ]]; then
    ...
    tmux set-option -g @tmux-status-line_netspeed_iface "$INTERFACE"
fi
